...
import { MedicalServicesService, MedicalServiceImportResult } from '../../inventory/medical-services.service';

@Component({
  selector: 'app-medical-service-import-dialog',
  standalone: true,
  imports: [
  ...
  template: `
    <h2 mat-dialog-title>Importar Servicios y Procedimientos</h2>
...
export class MedicalServiceImportDialogComponent {
  private ref = inject(MatDialogRef<MedicalServiceImportDialogComponent>);
  ...
  uploadError = signal('');
  result = signal<MedicalServiceImportResult | null>(null);
  ...
  upload(file: File): void {
    ...
    this.svc.importMedicalServices(file).subscribe({
  ...
  downloadTemplate(): void {
    ...
    this.svc.downloadMedicalServicesTemplate().subscribe({
      next: blob => {
        this.downloading.set(false);
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'medical_services_template.xlsx';
        a.click();
